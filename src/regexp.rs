...
fn automata_union(m1: &M, m2: &M, prefix: String) -> M {
    ...
    let mut delta: Delta = delta!(
        (q0.clone(), 'λ', prefixed_m1.q0.clone()),
...
pub fn automata_star(m: &M, prefix: String) -> M {
    ...
    let mut delta: Delta = delta!(
        (q0.clone(), 'λ', prefixed_m.q0.clone()),
        (q0.clone(), 'λ', f.clone()),
    ...
    for (s, a, ns) in to_delta(&prefixed_m) {
        if prefixed_m.f.contains(&s) { continue };
        delta.insert( (s.clone(), a.clone(), ns.clone()) );
    }
...
mod tests {
    ...
    fn star_test_case_1() {
        ...
        let m_expected = M::new(
            ...
            stateset!("01q0-01q1-0f0-01q0-0f0-0q0"),
            delta!(
                ("01q0-01q1-0f0-01q0-0f0-0q0", 'a', "01q0-01q1-0f0-01q0-0f0-0q0")
            )
        );
